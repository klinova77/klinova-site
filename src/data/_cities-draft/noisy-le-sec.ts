...
const city: City = {
  ...
  hubIntro: `<p>Pôle multimodal RER E / Tram T1, entre le Canal de l'Ourcq et le quartier de la Boissière : Noisy-le-Sec concentre des typologies variées qui demandent un suivi régulier. Klinova s'organise pour répondre aux besoins des copropriétés, gestionnaires et occupants sur l'ensemble de la commune.</p>
...
<p class="mt-4">Nous adaptons chaque intervention au <strong>rythme d'activité des bureaux</strong> et aux contraintes d'accès des copropriétés. Diagnostic du support, protection des plinthes, extraction contrôlée : chaque étape est planifiée pour limiter l'interruption des usages quotidiens.</p>`,
      uniqueDeepDive: `<h3>Zones de circulation intense</h3>
<p>Les couloirs, halls d'entrée et espaces de passage concentrent l'essentiel des salissures. Nous identifions la nature du textile — bouclé, velours ras ou dalle modulaire — puis appliquons un pré-traitement sur les traces marquées. L'aspiration préalable retire les particules libres avant injection.</p>

<h3>Surfaces de travail et espaces cloisonnés</h3>
<p>Les bureaux individuels, salles de réunion et recoins moins fréquentés reçoivent un traitement adapté à leur niveau d'encrassement. L'injection-extraction permet de retirer les résidus incrustés sans saturer le support. Les plinthes et bas de cloisons sont protégés pendant l'opération.</p>

<h3>Finitions et contrôle du séchage</h3>
<p>Après extraction maximale de l'humidité, nous vérifions l'homogénéité du rendu sur l'ensemble de la surface. Le temps de séchage dépend de l'épaisseur du textile et de la ventilation disponible. Des consignes de non-piétinement sont transmises pour garantir un résultat durable.</p>`,
      specificChallenges: [
        "Dalles textiles des bureaux de la Z.I. du Parc soumises à un passage quotidien soutenu.",
        "Moquettes aiguilletées des escaliers anciens du centre-ville, souvent étroits et difficiles d'accès.",
        "Taches de café et traces de semelles incrustées dans les zones de trafic des open spaces.",
        "Halls de copropriétés rénovés nécessitant un entretien régulier pour conserver leur aspect soigné.",
        "Stationnement du véhicule d'intervention compliqué en centre-ville aux heures de pointe.",
      ],
      faqAdditions: [
        {
          question:
...
<p class="mt-4">Une intervention structurée redonne à cet espace son usage quotidien. Diagnostic du support, protection des menuiseries, traitement ciblé et gestion de l'eau vers le bas : chaque étape vise un résultat visible sans créer de désagrément pour le voisinage. Les balcons situés en étages élevés ou exposés aux flux de circulation méritent une attention particulière sur les méthodes employées.</p>`,
      uniqueDeepDive: `<h3>Dépôts incrustés et joints encrassés</h3>
...
<p class="mt-4">La coordination avec les artisans, les agences immobilières ou les gardiens d'immeuble fait partie intégrante de l'organisation. Accès par digicode, créneaux imposés par l'état des lieux, stationnement contraint en centre-ville : chaque contrainte est anticipée pour respecter les délais convenus.</p>`,
      uniqueDeepDive: `<h3>Un logement prêt à être occupé ou visité</h3>
